...
fit.pl <- lm(MeanSI ~ poly(SNR, aSMR, degree = 4, raw = TRUE), data = SIcomb)###Create polynomial model of degree 4
save(fit.fd,fit.pl,fit.sx,fit.lw,fit.py, file = "SIModel.RData")

###Plot polynomial model####
zfit <- matrix(fitted(fit), ncol = 5)
...
slopes <- foreach(Spp = c("Pl","Sx","Bl","Cw","Hw","Fd","Lw","Py"), .combine = rbind) %do% {##foreach species
  sibec <- read.xlsx("SIBEC_for_Portfolio2.xlsx", sheet = 1)
  sibec <- sibec[sibec$TreeSpp == Spp,]
  sibec <- sibec[!is.na(sibec$BGCUnit),]
  ...
  foreach(eda = edaPos, .combine = rbind) %do% { ##foreach edatopic position
    ...
    temp <- data.frame(Species = Spp, Eda = paste(eda[1],eda[2],sep = "/"), Intercept = fit$coefficients[1],
...
zoneSelect <- unique(portBGC$MergedBGC)
...
dat <- dat[,c(2,5,239,230,183,184)]
dat$ID2 <- gsub("[[:space:]]","",dat$ID2)
mods <- list(Pl = fit.pl, Sx = fit.sx, Fd = fit.fd, Py = fit.py, Lw = fit.lw) ##list of polynomial models

##this loops calculates SI values for each species in each unit and averages them by unit####
SIFill <- foreach(Spp = c("Pl","Sx","Fd","Py","Lw"), .combine = rbind) %do% {
  sibec <- read.xlsx("SIBEC_for_Portfolio2.xlsx", sheet = 1)
  sibec <- sibec[sibec$TreeSpp == Spp,]
  sibec <- sibec[!is.na(sibec$TreeSpp),]
  
  climSlope <- read.csv("SI_Stats_TaveSM.csv") ###Or use slope dataframe from above
  slopeSub <- climSlope[climSlope$Species == Spp & climSlope$Eda == "C/5",] ##Can choose which edatopic position to use
...
write.csv(SIFill,"PredSIforPort.csv", row.names = FALSE)
